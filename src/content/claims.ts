...
export const claims: Claim[] = [
  ...
  },
  {
    ...
    publicationInstruction:
      'HOLD — Publish only with current certificate for exact legal entity, scope and validity.',
    evidenceNote:
      'NO EVIDENCE EXISTS, as at 14 September 2026, and none has ever been produced in this project. The founder supplied certificate detail for ISO/IEC 27001:2022 and ISO 9001 on 14 September 2026 and supplied NOTHING for Cyber Essentials Plus — no certificate number, no issuing body, no dates, no certifying body reference. This row exists so that the absence is stated rather than left to be inferred from a compound label that has now been partly released. The previous site asserted Cyber Essentials Plus in footer pills, on the default social card and in prose; all of it was withdrawn on 8 September 2026 for want of a certificate, and none of it returns on the strength of the two ISO certificates, which say nothing about it. The 7 September 2026 legal review recorded that the IASME register sat behind bot protection and concluded: not doubted, not verified. That is still the position. WHERE IT RENDERS as at 14 September 2026: nowhere as a claim by this company. CORRECTED THE SAME DAY — this sentence previously continued "and the same is true of the words \'Cyber Essentials\' anywhere on the published site", and that was FALSE. Measured from the built HTML: the words render on three pages, five times. Twice on /assurance and once on the homepage in the Pixelette Certified pill row, which is preceded in the same block by "What Pixelette Technologies does hold is published with its certificate numbers and dates on our certifications page"; once more on /assurance describing the readiness work Certified does for clients; and once on /certifications as this register\'s own row saying the standard is not held. Every one of those is a standard CERTIFIED HELPS A CLIENT REACH or an explicit statement of absence, never a claim by Pixelette Technologies Ltd, so THE CLAIM GATE IS INTACT AND NOTHING ON THE SITE NEEDS TO CHANGE. What needed to change is this note. A register that misdescribes its own estate is the precise failure its 8 September audit paragraph exists to prevent, and a reader who checked this sentence against the site would have found it wrong and had no way to know which half to trust. WHAT WOULD MOVE THIS ROW: a current Cyber Essentials Plus certificate for Pixelette Technologies Ltd with its certifying body and expiry date, at which point this row and `iso-cyber-essentials-badges` can both be released together.',
  },
  {
    id: 'clutch-rating',
  ...
  },
  {
    ...
    evidenceNote:
      'A wording gate rather than a badge: only the exact substantiated relationship may be described, and nothing that reads as Parliamentary endorsement. CORRECTED 8 September 2026. This note said "Not currently rendered anywhere in src/", which was false. It renders twice, both on /about (src/app/about/page.tsx): once as the answer to "What is Pixelette Technologies’ connection to UK AI policy?" in that page\'s `faqs` array, which `faqSchema(faqs)` also emits as FAQPage JSON-LD, so the claim is machine-readable as well as visible; and once as the body of the "Policy exposure" section. Both give the same relationship — the founder holds a shareholding in Big Innovation Centre, which acts as Secretariat to the All-Party Parliamentary Group on Artificial Intelligence — and both state in terms that it is not an accreditation, an endorsement or a partnership. That is the wording gate being met rather than breached, which is why the row stays HELD rather than moving: the gate is on the wording, so any edit has to be re-checked against the substantiated relationship. RE-RUN 18 September 2026, on the About rework. It now renders ONCE, as the body of the "Industry engagement" section on /about, and NOT in any machine-readable surface: the FAQ that carried the second copy was removed with the rest of the FAQ block on that page, so /about no longer emits FAQPage JSON-LD at all. That is a gate improvement rather than a loss, because JSON-LD is the channel where a claim is quoted back without the disclaimer standing next to it. The relationship sentence is unchanged from the wording that cleared this gate. The disclaimer was rewritten on founder instruction and is now MORE specific, naming Parliament and the APPG rather than saying only that the relationship is not an accreditation: "This relationship is not an accreditation, endorsement or partnership with Parliament or the APPG." Also removed on instruction, and worth recording because it was the only part that went beyond the substantiated fact: "That gives us visibility of UK AI policy as it forms." The shareholding is a fact; the visibility inferred from it was a benefit claim.',
  },
  {
