...
export const claims: Claim[] = [
  ...
  },
  {
    ...
    evidenceNote:
      'CORRECTED 11 September 2026. The status is deliberately unchanged; the note is not. This note used to read that src/content/clients.ts records a required `permission` field per row and that "every row reads UNCONFIRMED, so `approvedClients()` is empty", and that the names rendering anyway was "a deliberate open gate, not an oversight: removing seven clients from the homepage is the founder\'s decision, and the file says so". Both clauses stopped being true on 11 September 2026. WHAT EVIDENCE EXISTS. The founder was asked whether the seven names then rendering with no recorded permission should be hidden or kept, and answered "Keep them — I\'m confident we have the basis". On that basis, and on that alone, the seven rows in src/content/clients.ts — gowalkies, SIB360, One-Stop CCTV, Butter Smiles, Beowulf, CAST Perimeter and Lytics — were set to APPROVED on 11 September 2026, and `approvedClients()` returns all seven. The evidence IS that decision, attributed and dated in that file. No per-client release document exists anywhere in this project, this note does not assert one, and the founder\'s decision of 11 September 2026 is what would have to be produced if the basis for a particular name were ever challenged. An eighth name, Akashic Knowing, is NOT covered by it: it sits in `additionalClients`, nothing imports that array, this build has never published it, and it stays UNCONFIRMED because the founder was not asked about it. Nothing has been confirmed about any logo IMAGE. WHERE IT RENDERS, re-run on 11 September 2026 rather than inherited: the same seven names render as text wordmarks through `ClientLogos` on / (src/app/page.tsx) and /ai-engineering (src/app/ai-engineering/page.tsx). That component reads `clients` directly — not `approvedClients()`, and not this register — so the names do not render through this row, and this row\'s status gates none of them. Rendering the homepage with every row flipped back to UNCONFIRMED on 11 September 2026 produced byte-identical HTML, which is the check behind that sentence rather than an inference from it. No logo image renders anywhere: the artwork in public/logos is white-on-transparent and unusable on this light design. WHY THIS STAYS HELD, which is a decision and should be read as one. The founder approved seven named clients; this row governs a claim CLASS, "Named client logos and wordmarks". VERIFIED would assert that the class is cleared — covering the eighth name he was never asked about, every name added after today, and logo images for which there is neither usable artwork nor permission. Nor would it be inert: `publishedClaims()` feeds `ProofStrip`, so VERIFIED publishes a badge reading "Named client logos and wordmarks" in the Verified proof strip on / and under "Verified and published" on /certifications — the site advertising its own permission as a proof point. That was measured by rendering it on 11 September 2026, not reasoned about. HELD keeps the gate fail-closed for the next name and for the first image, which is what the instruction above asks of it, and leaves the per-name record where it belongs, in src/content/clients.ts. To move this row, the class needs the engagement and the public-use basis confirmed for every name and every mark it covers, not one decision about seven of them.',
  },
  {
