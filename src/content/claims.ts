...
export const claims: Claim[] = [
  ...
  },
  {
    ...
    evidenceNote:
      'Corrected 8 September 2026: this note said the names "carry no recorded permission basis", which is now only half true — src/content/clients.ts records a required `permission` field per row, and every row reads UNCONFIRMED, so `approvedClients()` is empty. What the note did not say is that the names render anyway. Seven client names render as text wordmarks today, through `ClientLogos` on / (src/app/page.tsx) and /ai-engineering (src/app/ai-engineering/page.tsx), which reads `clients` rather than `approvedClients()`. That is a deliberate open gate, not an oversight: removing seven clients from the homepage is the founder\'s decision, and the file says so. No logo image renders anywhere — the artwork in public/logos is white-on-transparent and unusable on this light design. Each name needs the engagement confirmed and the right to name it publicly recorded before it stays; the handoff also says never to use a logo purely because it appears in an old deck.',
  },
  {
